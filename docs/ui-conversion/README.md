...
| [UI-04](cards/04-membership-renewal.md) | Membership renewal | — | Last. Reuses autocomplete + upload patterns from 03. **Split into four PRs** (2026-09-15 amendment): PR 1 rejection read-model — SHIPPED as PR #52 (`c20c703`); PR 2 expired worklist read UI — SHIPPED as PR #53 (`2562af1`); PR 2b read-side remainder (gate/toggle, stat cards, รอตรวจสอบ/ปกติ table, slip viewer) and PR 3 write flows (closes #49) remain. |
...
1. **Fix `components.json` aliases** so the shadcn CLI writes to the real locations (current aliases point at non-existent `src/components`):
   ...
   After every `bunx shadcn@latest add <component>`, check the generated imports resolve (they should import `cn` from `src/shared/lib/utils/utils`) and that file placement matches the existing primitives in `src/shared/components/ui/` (alert-dialog, badge, button, card, combobox, dropdown-menu, field, input, input-group, label, select, separator, textarea, …).

   **ADR-0022 split of homes**: these aliases send *primitives* to `src/shared/components/ui` — correct, primitives stay shared. Feature-owned components, hooks, and client schemas go under the feature module instead (`src/modules/<feature>/{components,hooks,schemas}`), never into `src/shared/components/`.
...
3. **Bulk status change = sequential `PATCH`es** — no bulk endpoint exists; loop over selected members, then toast a summary (N ok / M failed). *(Superseded by item 8: no status writes from the members UI at all — kept as the historical record of why.)*
...
8. **Member status is never written from the members UI** — the mockup's bulk ปรับเป็นยังไม่ได้ต่ออายุ / ปรับเป็นปกติ buttons are dropped (still drawn in mockup v2, still dropped). The v1 wizard's status toggle no longer exists in mockup v2 — the wizard's renewal block now renders disabled "ระบบจะระบุอัตโนมัติ" fields in create mode and a read-only lock note in edit mode, agreeing with the API. `PATCH /api/v1/members/[id]` has no `status` field by design — Member Status is a lifecycle column owned by the renewal flow (update-member service preserves it; see CONTEXT.md *Renewal Cache Columns*). Status changes reach a member only via Renewal Review or a Manual Renewal Submission (card 04 UI). Display vocabulary (incl. `RESIGNED` → ลาออก) is the Status Badge term in CONTEXT.md; badges are staff-only, matching the mockup.
9. **Card 03 edit-wizard gap (3b-EDIT prerequisite) — RESOLVED by PR #47 (`9edd9ba`)** — the gap was: `PATCH` required a 13-digit `id_card_no`, but `GET /api/v1/members/[id]` returns only the Masked ID Card, and ADR-0012 null-stickiness covered only the five file-path fields — so an edit could not be saved without the admin re-typing the full ID number (echoing the masked value failed the 13-digit check with 400). Fixed in 3b-edit as recommended: null-sticky semantics extended to `id_card_no` (`null` or absent = keep the stored card; 13 digits when replacing it) — schema, service, tests, and the OpenAPI re-export all landed with PR #47. **Creation was never affected** — `POST` takes the full 13-digit number, which is why 3b-create shipped first without this.
10. **Mockup v2 wizard dispositions (card 03 resync)** — where the v2 wizard and the API disagree, the API wins: (a) six fields the mockup marks optional are API-required and get required markers + validation in the wizard — `nickname`, `date_of_birth`, `nationality` (keeps the ไทย default), `id_card_expiry_date` (domain rule ≥ today), `business.juristic_registration_no` (unconditionally, not juristic-only), `business.description`; (b) the หมวดธุรกิจ select is fed live by `GET /api/v1/business/categories` (`{id, name}`[] → id values, Thai labels) — the mockup's 14 numbered strings are vocabulary only; (c) the draft autosave (Member Form Draft, CONTEXT.md) is client-only localStorage with no API backing — create mode only, File objects excluded.
...
| Members (03) | `GET/POST /api/v1/members`, `GET/PATCH/DELETE /api/v1/members/[id]`, `POST /api/v1/members/file/upload`, `GET /api/v1/business/categories`; `GET /api/v1/members/file/presign` — **DEPRECATED in Apidog**, not implemented: expired private-file view URLs recover via a `GET /api/v1/members/[id]` refetch |

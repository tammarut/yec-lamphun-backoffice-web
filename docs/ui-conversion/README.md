...
9. **Card 03 edit-wizard gap (3b-EDIT prerequisite only)** — `PATCH` requires a 13-digit `id_card_no`, but `GET /api/v1/members/[id]` returns only the Masked ID Card, and ADR-0012 null-stickiness covers only the five file-path fields. As-is, an edit cannot be saved without the admin re-typing the full ID number (echoing the masked value fails the 13-digit check with 400). Recommended fix for the 3b-edit session: extend null-sticky semantics to `id_card_no` (`null` = keep existing) — schema, service, tests, OpenAPI + Apidog re-export — before building the edit form. **Creation is unaffected** — `POST` takes the full 13-digit number, which is why 3b-create ships first without this.
...
| Members (03) | `GET/POST /api/v1/members`, `GET/PATCH/DELETE /api/v1/members/[id]`, `POST /api/v1/members/file/upload`, `GET /api/v1/members/file/presign` (3b-edit: re-mints expired private-file view URLs), `GET /api/v1/business/categories` |
